...
def main():
    ...
    async def on_message(message):
        ...
        if message.content.startswith('!Blackjack'):
            #await message.channel.send('Starting gold?')
            #msg = await client.wait_for('message')
            #goldstart = int(msg.content)
            ...
            while player_in:
                ...
                await message.channel.send(
                    current_score_str %
                    (hand_value(player_hand)[0], player_hand))
                ...
                if player_in:
                    ...
                    if response:
                        ...
                        await message.channel.send('You draw %s' %
                                                   new_player_card)
                    else:
                        player_in = False
            ...
            if player_score <= 21:
                dealer_hand_string = '''\nDealer is at %s\nwith the hand %s\n'''
                await message.channel.send(dealer_hand_string %
                                           (dealer_score_label, dealer_hand))
            else:
            ...
            while hand_value(dealer_hand)[1] < 17:
                ...
                await message.channel.send('Dealer draws %s' % new_dealer_card)

            dealer_score_label, dealer_score = hand_value(dealer_hand)

            if player_score < 100 and dealer_score == 100:
                await message.channel.send('You beat the dealer!')
            elif player_score > dealer_score:
                await message.channel.send('You beat the dealer!')
            elif player_score == dealer_score:
                await message.channel.send('You tied the dealer, nobody wins.')
            elif player_score < dealer_score:
                await message.channel.send("Dealer wins!")
